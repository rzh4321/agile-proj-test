...
const calculateMatchScore = (store: Store, filters: FiltersType): number | null => {
  ...
  // category match
  const categoryMatch = store.categories.some(cat => filters.category.includes(cat));
  if (filters.category.length > 0 && categoryMatch) {
    score += weights.category;
  }

  // price range match
  if (filters.priceRange.length > 0 && filters.priceRange.includes(store.priceRange)) {
...
export const suggestStores = (stores: Store[], filters: FiltersType): Store[] => {
  ...
  const scoredStores = eligibleStores.map(store => ({
    store,
